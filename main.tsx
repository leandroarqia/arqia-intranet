                  ...
                  {activeView === 'dashboard' && (
                    <motion.div variants={itemVariants}>
                      <h2 className="text-3xl font-bold mb-2">{(() => {
                        ...
                        return <>{s}, <span className="text-[#00D1C1]">{user.name}</span>!</>;
                      })()}</h2>
                              ...
                              {selectedClient === String(base.id) && (
                                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                                  <motion.div initial={{ scale:0.9,opacity:0 }} animate={{ scale:1,opacity:1 }} exit={{ scale:0.9,opacity:0 }}
                                    ...
                                    {clients.filter(c=>(c.codigo_cliente||'')===(base.codigoCliente||'')).length === 0 ? (
                                      <p className="text-white/40 text-sm text-center py-6">Nenhum dispositivo encontrado para esta base.</p>
                                    ) : (
                                      <table className="w-full text-left text-sm border-collapse">
                                        ...
                                        <tbody>
                                          {clients.filter(c=>
                                            (c.codigo_cliente||'') === (base.codigoCliente||'') &&
                                            ((c.iccid||'').includes(searchQuery)||(c.imei||'').includes(searchQuery))
                                          ).map((c,i)=>(
                                            <tr key={i} className="border-b border-white/5 hover:bg-white/5 transition-colors">
