...
import TRX16Animation from './components/TRX16Animation';
                  ...
                  {activeView === 'dashboard' && (
                    <motion.div variants={itemVariants} className="flex items-start gap-8">
                      <div className="flex-1 min-w-0">
                        ...
                        {isUserAdmin && !hasSupabase && (<div className="mt-6 p-4 bg-yellow-900/20 border border-yellow-700/40 rounded-xl"><p className="text-yellow-300 font-semibold text-sm mb-1">⚠️ Banco de dados não configurado</p><p className="text-yellow-200/70 text-xs">Os dados estão salvos apenas no seu navegador.</p></div>)}
                      </div>
                      <TRX16Animation />
                    </motion.div>
                  )}
                  {activeView === 'clientes' && (
                    <motion.div variants={itemVariants} className="bg-[#0C1635]/80 p-6 rounded-2xl border border-white/10">
                      <div className="mb-4 flex gap-4">
                        <input type="text" placeholder="Pesquisar (Nome, ICCID ou IMEI)..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="flex-grow bg-[#080E24] border border-white/10 rounded-lg py-2 px-4 text-white placeholder-white/30 focus:border-[#00AEEF] outline-none transition-colors" />
